...
//	private ArrayList<MemberDTO> list;
	...
	public MemberDisp() {
		...
		table = new JTable();
		model = new DefaultTableModel(table.getColumnCount(), 0) {
			@Override
		...
		cbxId.addActionListener(this);
		cbxName.addActionListener(this);
		cbxTel.addActionListener(this);
		cbxBirth.addActionListener(this);
	...
	public void addColumn() {		
		int cnt= 0; 
		String[] colName = {"아이디", "이름", "전화번호", "생년월일"};
		
		if(cbxId.isSelected()) {model.addColumn("아이디"); cnt++;	}
		else if(cbxName.isSelected()) {	model.addColumn("이름"); cnt++;	}
		else if(cbxTel.isSelected()) {model.addColumn("전화번호"); cnt++;}
		else if(cbxBirth.isSelected()) {model.addColumn("생년월일"); cnt++;}
	...
	public void removeColumn() {
		...
		int idx = 0;
		if(!cbxId.isSelected()) idx = 1;
		if(!cbxName.isSelected())	idx = 2;
		if(!cbxTel.isSelected())	idx = 3;
		if(!cbxBirth.isSelected())	idx = 4;
...
//		System.out.println("table" + table.getColumnCount());
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == cbxId) {
			
		} else if(e.getSource() == cbxName) {
			
		} else if(e.getSource() == cbxTel) {
			
		} else if(e.getSource() == cbxBirth) {
